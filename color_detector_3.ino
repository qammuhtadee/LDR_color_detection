...
void scanHardware() {
  ...
  int xR = results[0] - blackRef.r;
  int xG = results[1] - blackRef.g;
  int xB = results[2] - blackRef.b;
  ...
  int rangeR = whiteRef.r - blackRef.r;
  int rangeG = whiteRef.g - blackRef.g;
  int rangeB = whiteRef.b - blackRef.b;
  ...
  // normalize to 0–1
  float nR = (float)xR / (float)rangeR;
  ...
  // scale to 0–255
  currentScan.r = constrain((int)(nR * 255.0), 0, 255);
  currentScan.g = constrain((int)(nG * 255.0), 0, 255);
  currentScan.b = constrain((int)(nB * 255.0), 0, 255);
}
